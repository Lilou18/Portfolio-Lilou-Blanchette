...
export class Player {
    ...
    setupScrollInput(scrollState) {
        window.addEventListener('wheel', (e) => {
            ...
            // Touchpad : smaller values (< 50 generally)
            // Molette : bigger values (> 50)
            ...
            if (isTouchpad) {
                // Touchpad mode: normalize deltaY to prevent excessive movement
                const direction = Math.sign(e.deltaY);

                // Normalize the deltaY to 0-1 range (touchpad typically gives 5-50)
                const normalizedDelta = Math.min(Math.abs(e.deltaY) / 50, 1);

                scrollState.scrollDelta = direction * normalizedDelta * 0.4;
    ...
    */
    ...
    /**
     * Update player speeds and jump force based on the level scaling.
     * 
     * @param {number} scaleX Horizontal scale factor of the level
     * @param {number} scaleY Vertical scale factor of the level
     */
    ...
    update(keysPressed, jumpState, scrollState, baseColliderShape, fallColliderShapeRight, fallColliderShapeLeft) {
        this.k.onUpdate(() => {
            ...
            // If scroll movement is too small we stop the movement
            if (Math.abs(scrollState.scrollDelta) < 0.05) {
                scrollState.scrollDelta = 0;
                scrollState.isScrolling = false;
            }
